...
因此：生产 scorer、Prompt、Graph、预算、目标和 `current_best` 代码本次均未修改；是否把生产 acceptance 换成 strict-total 需要独立决策与后续真实模型/人工证据。本实验是 independent no-model diagnostic，不是 D058/D059 冻结 benchmark，不能使 F09 passing；contact sheet 代理看片只是工程分析，不构成独立人工盲评。
...
- 模型调用：0
- Arm G：`geometry_mask_loss` 优先、`total_loss` 其次的字典序严格改善（既有 geometry 局部搜索语义）
...
1. 是否把生产 acceptance 从 geometry-first 字典序改为 strict total-loss，需要独立 ADR 决策；本实验只提供固定 7 例无模型证据，生产代码本次未改。
