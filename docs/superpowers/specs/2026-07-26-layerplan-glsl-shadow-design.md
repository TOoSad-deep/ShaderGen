...
- 授权范围：D083 只授权 shadow 实验与本文档级设计基线；不修改生产 Graph、代码、API、FEATURE 状态、scorer、预算或 `current_best` 安全边界
...
| 阶段 | 内容 | 完成判据 |
|---|---|---|
| 1（本次） | 修订版设计基线：D083 修订 + 本文档 + PROGRESS 刷新 | `make docs-check` 与 `git diff --check` 通过；无生产改动 |
