...
| F09 | 用户上传 PNG 后，scene_mvp 最小骨架执行感知、ShaderGraph Author、specialized WebGL1 编译/渲染、复合评分、node-id 有界优化和 typed layer Refine，并返回 GLSL、最终 Render、指标与 trace。 | `make check && uv run pytest -q tests/integration_tests && make test-scene-mvp-ui` | active | 2026-07-24：`shader_graph_v1` 已成为默认产品真相源；Initial、typed layer patch、CandidateSnapshot、多 program cache、node/layer 参数 block、manifest/API/Layer inspector 已贯通。生产 `dashscope:qwen3.7-plus` 三个 Initial 小样例和一次 Refine 直连成功，脱敏 repair 诊断有效；但三个 Initial 候选均输给 fallback，仍缺 Boolean/8-layer 固定样例、兼容能力决策、durable benchmark 和独立人工门禁。 |
