...
| F09 | 用户上传 PNG 后，系统可以在 WebGL1 无贴图契约下执行有界的分析、生成、真实渲染、评分和修订闭环，并返回 current_best GLSL 与证据。 | `make check && uv run pytest -q tests/integration_tests && npm --prefix frontend run e2e:procedural-v1 && npm --prefix frontend run e2e:memory && make test-scene-mvp-ui && make benchmark-ai-off && make benchmark-gate BENCHMARK_OUTPUT=<run-dir> HUMAN_REVIEW=<review.json>` | active | 2026-07-22：并行 `scene_mvp` 已贯通模型/fallback 真实仲裁、14/16 WebGL1 uniform vectors 的 v2 packed 三槽模板、rim/arc/line 独立几何、累计式确定性优化、48/96/160 分档 draw 预算、局部复合 loss、Backend/Artifact 和前端；Chromium 无模型 fast 冒烟与专用浏览器 E2E 通过。CMA-ES/2000 draw、真实模型质量 benchmark 和新人工门禁尚未完成；既有正式 run 人工偏好仍为 30%（门槛 50%），因此 F09 继续 active、灰度 no-go。 |
