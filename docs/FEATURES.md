...
| F09 | 用户上传 PNG 后，scene_mvp 最小骨架执行感知、ShaderGraph Author、specialized WebGL1 编译/渲染、复合评分、node-id 有界优化和 typed layer Refine，并返回 GLSL、最终 Render、指标与 trace。 | `make check && uv run pytest -q tests/integration_tests && make test-scene-mvp-ui` | active | 2026-07-24：`shader_graph_v1` 已成为默认产品真相源；Initial 完整文档、单 typed layer patch、不可变 CandidateSnapshot、run-scoped 多 program cache、node/layer 参数 block、manifest/API/只读 Layer inspector 已贯通。真实 WebGL1 产品集成与一条 Kimi Code 输出注入 canary 通过；生产 Provider Gateway 因无 `.env` 未直连，且仍缺固定多样例 benchmark、兼容能力决策和独立人工门禁。 |
