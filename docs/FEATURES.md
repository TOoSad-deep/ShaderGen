...
| id | 行为 | 验证 | 状态 | 证据 |
|---|---|---|---|---|
| H01 | 新 agent 会话可以只通过仓库文件理解项目用途、命令、目标架构、决策、当前进度和下一个功能。 | `make check` | passing | 2026-07-21：目标架构权威关系同步后 `make check` 通过：414 个单元测试通过，docs-check、LangGraph validate（1 个 graph）和前端构建均成功。 |
