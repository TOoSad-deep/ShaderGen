...
| F03 | DSL 节点图和 Renderer 可以产出 GLSL 以及可渲染画面。 | `uv run pytest tests/unit_tests/test_dsl_renderer.py && npm --prefix frontend run build` | not_started | 来自目标架构的核心处理层。 |
...
| F09 | 用户上传 PNG 后，scene_mvp 最小骨架执行感知、Scene Author、固定模板、真实 WebGL1 渲染、复合评分和有界优化，并返回 GLSL、最终 Render、指标与 trace。 | `make check && uv run pytest -q tests/integration_tests && make test-scene-mvp-ui` | active | 2026-07-23：产品已收敛为唯一 `png_to_shader_min` Graph；V1 Graph/Node/Parser/Prompt/Service、Backend/Frontend 分流、旧 Artifact/Memory API 和 V1 benchmark 已删除。当前 tile guard offline replay 不采用，仍需 live acceptance A/B、新冻结 benchmark、真实模型固定样例和独立人工门禁。 |
