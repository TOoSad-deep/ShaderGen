...
| H02 | 开发者可以通过独立 Node Lab 服务和 `/lab` 工作台接入任意显式 Provider，而产品 Backend 不隐式加载 Node Lab。 | `uv run pytest tests/unit_tests/test_node_lab_schemas.py tests/unit_tests/test_node_lab_http_schemas.py tests/unit_tests/test_node_lab_http_routes.py tests/unit_tests/test_node_lab_service.py && npm --prefix frontend run build` | passing | 2026-07-27：HTTP transport 已收拢为 `nodelab.http`，Schema 与 Route 均按资源职责拆分并保持稳定 façade/OpenAPI；受信任 factory、空安全默认 Application、CLI/HTTP 契约与工作台构建保持通过。工作台已补齐整宽空目录引导、离线重试、在线目录刷新、分动作忙碌反馈和窄屏退化，并以临时 Provider 实测 LabRun 创建、节点执行、DAG/输出和 Artifact 上传。旧 `nodelab_service` 不再进入 wheel，旧 V1 Adapter、manifest 与 benchmark 脚本未恢复。 |
