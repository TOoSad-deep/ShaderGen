...
D086 在单 run 之上新增冻结 suite 层：`benchmarks/layerplan_glsl_shadow/manifest_v1.yaml` 绑定四张参考图、instruction、共享预算和两轮 `AB/BA` 顺序，`gate_v1.yaml` 绑定 manifest SHA、两种完整配置指纹、metric 与看结果前的判定阈值。`scripts/run_layerplan_glsl_shadow_suite.py` 默认拒绝真实模型，只有显式 `--allow-live-model --output-root <私有目录>` 才会顺序运行全部样本；每个单 run 先经 `verify_shadow_run`，suite 再按同样本同轮的 `B-A` loss、顺序效应和 inconclusive 计负规则聚合，并以原子私有报告落盘。`--verify <shadow-suite-*目录>` 会复验 suite 和全部引用 run。当前报告仍为 `local_private_not_registered`；自动 gate 即使通过也保持 `no_go_pending_human_and_durable`。生产 `png_to_shader_min` 的 12 节点拓扑与 ShaderDocument/Compiler 路径不变。
