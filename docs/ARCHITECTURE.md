...
`png_to_shader_min_graph.py` 是显式 `scene_mvp` 实验路径：12 个节点和 3 个纯路由函数依次完成输入登记、确定性感知、严格 scene、固定模板、真实 WebGL1 渲染、局部复合评分、基础/特征参数搜索、final Artifact 与阶段 trace。Graph Builder 通过既有 `LLMGateway` 注入 Model Author：Initial 严格解析完整 MinScene，并与感知 fallback 分别真实渲染后择优；Refine 只接受一个 path/operation/value 白名单 patch，从只读 `current_best` 派生独立候选 branch。系统向 Refine 提供固定 4×4 worst-tile 的 MAE、`rendered-reference` 亮度/RGB signed bias、主导 metric、active feature 与最近拒绝摘要；Patch 只保存 operation、feature id/type 和规范 SHA-256，不持久化完整 value。每个非重复合法 Patch 最多消费现有 render budget 中的 12 次 draw（raw 1 次、局部成熟最多 11 次）：add/replace feature 只调该 feature，replace color field 只调颜色场，remove 只重评分；matured candidate 严格改善才原子提交，否则整支回滚。`png_to_shader_min_scene_v3` 严格区分 solid/radial/linear 并最多接受 4 个 feature；`png_to_shader_min_template_v3` 为每槽使用 shape/color-power 2 个 `vec4`，连同 scene/类型元数据和 `u_resolution` 精确占用 15/16 active fragment uniform vectors，并为 rim/shadow/polar_arc/edge_line/gaussian_lobe/glow 提供不同主体内外语义。首次 feature queue 来自实际获胜 scene 的稳定 id；Refine 后经 no-op base 过桥，不再重跑完整 sweep。只有 `min_scene_composite_v3` 的 global/foreground/background/geometry/edge/worst-tile 复合 loss 严格改善才能提交，整图 MAE 保留为诊断。`src/agent/app/config/png_to_shader_min.yaml` 同时冻结 run classification、实验 ID、报告版本、配置 SHA-256、MAE/loss 目标与硬预算；当前 `0.04/0.02` 和 `48/2/1`、`96/4/2`、`640/9/9`、manual `1000/32/30` 明确属于独立实验，声明冻结 benchmark 时若漂移 D058/D059 的 `0.08/0.04` 与 `48/2/1`、`96/4/2`、`160/6/3` 或携带 manual 则启动失败。Service 按 `9+2F+6R` 推导 run 级 Graph recursion limit，high/manual 分别注入 69/201，合法路径不会先撞框架上限；意外 `GraphRecursionError` 仍 fail-closed。无密钥时模型错误收敛到安全 fallback。同一 run 的固定 WebGL1 模板由 `prepared_uniforms_v1` 只编译/链接一次，候选只热更新 typed uniform。最终 GLSL 仍烘焙常量以保持自包含和旧 `render()` 兼容；CMA-ES/2000 draw、geometry 语义拆分、真实模型固定 7 例和人工偏好仍是明确缺口。Backend 按 `generation_mode=procedural_v1|scene_mvp` 分流，manual 只允许后者并在前端条件展示；两个路径共用 project/run 日志和 final-render/metrics/manifest 白名单。两个 Graph 的 Service 都共享 run 级 Renderer registry，并在 `finalize` 与外层 `finally` 幂等清理。旧基础对话 Graph、legacy 生成/独立 Review 图及其产品入口已下线。
