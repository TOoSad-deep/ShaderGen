...
> 归档状态：截至 D099 的完整历史决策快照，仅用于审计。本文中的状态索引、后续任务和约束均不再直接作用于当前开发。
...
| 决策 | 状态 | 后续决策 | 当前解释 |
|---|---|---|---|
| D001 | superseded | D050 | SVG 降为最初设计参考，不再是当前架构权威。 |
| D002 | superseded | D010、D037 | 最小事实源已演进为模块就近文档和有界交接。 |
| D003 | accepted | — | 保留 `src/agent/`、顶层 Backend 与 Frontend 的 monorepo 布局。 |
| D004 | accepted | — | 应用层、Agent、领域核心和工具层保持分层 monorepo 边界。 |
| D005 | updated | D043 | 幂等业务 SQL 原则保留，资源现由显式包承载。 |
| D006 | updated | D035、D036 | Backend 只调用 Agent 公共 service；旧 service 路径已删除。 |
| D007 | updated | D017、D036、D038 | `agent.app.*` 结构保留，旧 Graph/models/Node 路径只属历史。 |
| D008 | accepted | — | Backend 保持 API、Service、Database、Middleware 分层。 |
| D009 | superseded | D036 | 独立 review API 与浏览器先渲染流程已删除。 |
| D010 | accepted | — | 模块边界继续由相邻 `ARCHITECTURE.md` 维护。 |
| D011 | updated | D036 | Parser 纯边界保留，旧 generation service 已删除。 |
| D012 | updated | D017、D027 | reasoning 默认不进入业务输出，当前实现在 LLM Gateway。 |
| D013 | updated | D017 | provider/model-family 分离保留，旧 models 路径已迁移。 |
| D014 | superseded | D017、D036 | 旧 runtime-context 模型配置已删除。 |
| D015 | superseded | D016、D027、D036 | 默认采集、打印和写入 reasoning 的行为已取消。 |
| D016 | superseded | D017、D027、D036 | 旧生成/评审 Node 工厂及 config 已删除。 |
| D017 | accepted | — | 模型访问统一经过 LLM Gateway 中立边界。 |
| D018 | updated | D036、D044、D068 | Memory/checkpoint 实现休眠保留，当前产品不接入。 |
| D019 | superseded | D036、D066 | 受限自由 GLSL V1 路径已删除。 |
| D020 | updated | D066、D097 | 真实 WebGL1 仍是执行事实层，旧 M1 结构已删除。 |
| D021 | superseded | D036、D066 | 旧 M2 角色与修复流程已被后续路径替代。 |
| D022 | updated | D070、D097 | best Artifact 与确定性路由仅适用于保留的 Graph fallback。 |
| D023 | superseded | D036、D066 | V1 产品路径、Artifact 与项目 Memory API 已删除。 |
| D024 | superseded | D076、D098 | 旧冻结门禁与盲评流程不再驱动当前开发。 |
| D025 | updated | D092 | 受限归一化与安全失败原则保留，具体契约由 direct v2 更新。 |
| D026 | updated | D066、D097 | HTTP 先定义公开契约的原则保留，旧 Metrics 形态已删除。 |
| D027 | updated | D036 | 可靠性与安全账本原则保留，legacy 兼容部分已删除。 |
| D028 | superseded | D032、D038、D065 | 旧 Node Lab Adapter 与快照协议已退役。 |
| D029 | superseded | D065、D076 | 旧 V1 benchmark pair 门禁已退役。 |
| D030 | superseded | D066、D076 | measurement affine 旧候选根已随 V1 删除。 |
| D031 | superseded | D065、D076、D098 | 旧人工盲评证据链只保留历史审计价值。 |
| D032 | superseded | D038、D065 | 旧 Node Lab Provider 已退役。 |
| D033 | updated | D076、D098 | 自动 objective 不等同人工偏好；当前不默认启动质量实验。 |
| D034 | updated | D038、D066 | 按职责拆分原则保留，旧 V1 Node 实现已删除。 |
| D035 | updated | D036、D044 | 薄 Route、Backend Service 与资源清理原则保留。 |
| D036 | superseded | D066 | PNG-to-Shader V1 产品入口已被删除。 |
| D037 | updated | D098、D099 | `PROGRESS.md` 只保留有界当前交接，不做逐会话日志。 |
| D038 | superseded | D066 | V1 功能命名空间已随旧路径删除。 |
| D039 | superseded | D066 | canonical V1 Validator/Renderer 契约已退役。 |
| D040 | superseded | D076、D098 | 旧 benchmark 预算扣减规则不再驱动当前验证。 |
| D041 | updated | D098 | durable/partial 区分保留，仅在明确证据或发布工作中使用。 |
| D042 | updated | D098 | 严格 CI 仍用于里程碑，不是普通改动默认门禁。 |
| D043 | accepted | — | 包根惰性导出，Backend SQL 使用显式资源包。 |
| D044 | accepted | — | Backend 在组合根冻结配置并补偿清理资源。 |
| D045 | updated | D076、D098 | 离线 benchmark 与在线 Service 分离；当前默认休眠。 |
| D046 | updated | D065 | 统一前端 API Client 保留，旧 Node Lab 明细部分已退役。 |
| D047 | superseded | D049、D050、D066 | V2–V5 递进计划已退役。 |
| D048 | superseded | D066 | 并行最小 Graph 验证阶段已经结束。 |
| D049 | superseded | D050、D066、D099 | 目标架构详细版降为历史目标参考。 |
| D050 | accepted | — | 最初 SVG 只作历史设计参考。 |
| D051 | superseded | D066 | 固定模板垂直切片已被后续产品收敛替代。 |
| D052 | updated | D066、D070、D097 | prepared Renderer 复用原则保留在 Graph fallback。 |
| D053 | superseded | D070 | 完整 scene/typed patch 旧 Author 契约已被 ShaderGraph 替代。 |
| D054 | superseded | D070 | 固定模板小预算参数搜索不再是当前默认表示。 |
| D055 | superseded | D070 | 旧 MinScene 多特征与局部 loss 加固路径已结束。 |
| D056 | superseded | D070 | packed 三槽模板硬边界已被 ShaderGraph 替代。 |
| D057 | accepted | — | 运行进度继续使用有界进程内事件与增量轮询。 |
| D058 | superseded | D070 | 固定模板扩展已被动态 Compiler 路径替代。 |
| D059 | updated | D070、D098 | 配置集中管理原则保留，旧模板 benchmark 预算不再适用。 |
| D060 | updated | D070、D097 | 合法预算推导安全上限保留在 Graph fallback。 |
| D061 | superseded | D070 | 旧 MinScene typed Patch 成熟流程已退役。 |
| D062 | updated | D070、D098 | manual 预算只属旧独立实验，不是当前默认档。 |
| D063 | superseded | D076、D098 | 旧 tile guard replay 结论只保留历史价值。 |
| D064 | updated | D065 | 共享依赖解耦保留；旧 Node Lab 已退役。 |
| D065 | superseded | D066、D067 | 旧 Node Lab 退役事实保留；现有通用 Node Lab 由 D082 重建。 |
| D066 | updated | D067、D070 | V1 可执行链路删除有效，后续表示和产物策略另有更新。 |
| D067 | accepted | — | 仅按用户针对精确范围的明确授权清理历史本地产物。 |
| D068 | accepted | — | Memory/checkpoint 与 PostgreSQL 数据休眠保留。 |
| D069 | updated | D070、D097 | ShaderGraph 已成为隔离 fallback，而非整体默认引擎。 |
| D070 | updated | D097 | 有界 ShaderGraph 是旧引擎和 fallback 真相源，不再是无 policy 默认。 |
| D071 | superseded | D098 | 跨分支参数优化 TODO 不再自动进入当前任务。 |
| D072 | updated | D073、D098 | A/B 结论仅作历史证据，不触发当前实验。 |
| D073 | updated | D070 | strict total-loss 只覆盖保留的 legacy/Graph 语义。 |
| D074 | updated | D070、D076 | 私有 replay 安全边界只保留在 legacy Builder。 |
| D075 | superseded | D076 | 12/32 maturity 只适用于旧候选空间。 |
| D076 | updated | D098 | 停止旧 Feature 质量实验；当前不默认重建 benchmark。 |
| D077 | accepted | — | ShaderGraph compile 上限按 run 预算推导。 |
| D078 | accepted | — | 无效 ShaderGraph Refine 走 no-op，不重建参数队列。 |
| D079 | updated | D080 | kimi provider 保留，model family 已独立。 |
| D080 | accepted | — | kimi family 固定 temperature=1，默认模型为 `kimi:k3-256k`。 |
| D081 | accepted | — | kimi reasoning effort 由环境变量控制，默认 low。 |
| D082 | accepted | — | 通用 Node Lab 是独立可选开发工具，不恢复旧 V1 插件。 |
| D083 | accepted | — | Node Lab HTTP transport 位于 `nodelab` 命名空间。 |
| D084 | accepted | — | Node Lab HTTP Schema 按资源拆分并保留 façade。 |
| D085 | accepted | — | Node Lab HTTP Route 按资源拆分并锁定 OpenAPI。 |
| D086 | updated | D097 | Node Lab 的 `scene_mvp` 投影调试保留，但非默认产品路径。 |
| D087 | superseded | D097 | shadow 设计阶段已结束，direct 已进入当前默认开发路径。 |
| D088 | updated | D098 | 隔离 A/B harness 和结果保留，默认不运行或扩建。 |
| D089 | accepted | — | 前端只展示后端可证明的运行阶段。 |
| D090 | updated | D098 | 冻结 suite 只作历史协议；新实验必须由用户明确触发。 |
| D091 | superseded | D094、D097、D098 | 首轮 no-go 只属历史 suite，不阻塞当前 direct。 |
| D092 | accepted | — | direct GLSL v2 的共用契约稳定性实现继续有效。 |
| D093 | updated | D098 | v2 身份冻结只用于历史复验，不是日常门禁。 |
| D094 | updated | D097、D098 | 自动门禁结果保留，不阻止单环境 direct。 |
| D095 | updated | D097、D098 | attempt 隔离、fallback、显式 policy 和 kill switch 保留。 |
| D096 | updated | D097、D098 | 盲评与 bundle 只作历史证据，不是当前完成条件。 |
| D097 | accepted | — | 无 policy 时使用无授权 `direct_default`，失败隔离回退旧引擎。 |
| D098 | accepted | — | 未上线单人开发采用风险分级、聚焦验证和显式质量治理。 |
| D099 | accepted | — | 实时文档优先；历史计划、spec、analysis 和目标方案必须首屏标注状态。 |
...
## D086 - 当前 scene_mvp 以 Artifact 投影接入独立 Node Lab

- 日期：2026-07-27
- 决策：在生产侧 `agent.app.nodes.png_to_shader_min.node_lab` 新增 `scene_mvp` 显式 Provider/Executor，登记当前 Graph 的 12 个稳定 Node ID，但不编译或执行 Graph；`agent.app.services.node_lab` 作为受信任 factory 组合根，按服务配置选择具体 LLM Gateway 并创建 Application，Node 命名空间继续只依赖中立 `LLMGateway` 契约。普通节点使用 `deterministic` binding；`author_initial`/`author_refine` 同时提供 AI-off deterministic 与 real binding，AI-off 强制把可用模型预算收敛到已消费值，real 继续要求 `NODELAB_REAL_MODEL_ENABLED=true` 和请求 `allow_model_call=true`。Lab State 只保存 JSON-safe 值：参考图、目标 RGB、候选 Render 和 final 文件使用同一 LabRun 的不透明 Artifact；`current_best` 保存 ShaderDocument、Compiler 指纹、active block、指标、provenance 与 Render Artifact ID，下一 Step 校验后重新编译并恢复 `ShaderGraphCandidateSnapshot`。Renderer registry 在同一服务 Application 内跨 Step 复用，由 `finalize` 幂等关闭。
- 原因：当前生产 Node 是由 `make_shader_graph_nodes()` 组合出的闭包，依赖 Artifact Store、Renderer registry 与 Gateway，且 State 包含 bytes、NumPy 数组、Compiler 产物和不可变 CandidateSnapshot，不能用通用 `DirectNodeExecutor` 安全直调。把领域 hydration/projection 留在 Agent 生产命名空间，可保持 `nodelab` 对 Agent/ShaderForge 零依赖，同时让工作台真正调试当前产品实现而不是复制节点语义。
- 影响：启动当前 Provider 需要设置 `NODELAB_APPLICATION_FACTORY=agent.app.services.node_lab:create_application`；默认空 Application、产品 Backend 隔离、HTTP 路径、Graph 节点/边/路由、scorer、预算和 `current_best` 生产安全语义均不改变。新增单元测试锁定 12 节点目录、Artifact hydration、NPY 投影、路由、真实 factory 导入和 real fail-closed，AI-off 集成测试覆盖 initialize、感知、fallback Author、Compiler、Renderer、优化、路由、Refine no-op 与 finalize Artifact。旧 V1 Adapter、Fixture、manifest、benchmark 和证据没有恢复；真实模型与真实 Chromium 工作台仍需操作者显式启动和授权。
## D087 - LayerPlan + 直接 GLSL Author 仅以非权威 shadow 实验接入，第一阶段只交付修订版设计基线
...
## D089 - 前端运行可观测性收敛为单一可测试阶段视图模型，不伪造后端未提供的进度
...
- 实现更新（2026-07-27）：同一协议冻结后新增 `run_layerplan_glsl_shadow_suite.py` 与 suite service，按 manifest 顺序执行全部样本/轮次；每个 run 必须先通过原 D092 verifier 且与 sample/reference/instruction/config/order 精确绑定，随后才按配对 `B-A`、样本中位数、AB/BA 方向一致性和 inconclusive 计负聚合。suite 报告采用私有 staging + 原子 rename、0600/0700、内容寻址目录，并可递归复验全部引用 run；自动结论只可能是 `no_go_automatic_gate_failed` 或 `no_go_pending_human_and_durable`。人工盲评包、durable registry 与生产晋升仍未实现。

## D091 - 首轮真实 LayerPlan suite 自动 gate 失败，生产保持 no-go

- 日期：2026-07-27
- 决策：接受 D094 冻结 suite `shadow-suite-43a0748fa395` 的自动结论 `not_supported`，生产决策为 `no_go_automatic_gate_failed`。本轮不生成晋升用人工盲评、不登记为 durable 证据、不修改 D070 ShaderDocument/Compiler、Graph、scorer、预算或 `current_best`。下一实验增量先提高 Arm A/B 共用 direct GLSL Initial/Refine/repair 对 `webgl1_static_no_texture_v1` 的遵循稳定性；不得放宽 Validator、静默修补越权 GLSL或把 LayerPlan 直接接入接受谓词。Prompt/repair/实现变化后必须升级版本并重新冻结 manifest/gate/实现身份，不得覆盖 v1 run。
...
## D092 - direct GLSL v2 先提高两臂共用契约稳定性
...
- 原因：D095 的 3/4 inconclusive 主要来自两臂共用 direct GLSL 契约遵循不稳定；若只改 LayerPlan 臂或放宽 Validator，会污染 A/B 控制变量并掩盖模型失败。把 runner 已执行的 canonical safety 前移到 Author Parser，才能让同一次有界 repair 处理真实阻塞规则，同时不引入确定性 GLSL 静默改写。
...
## D094 - direct GLSL v2 自动门禁通过后仍保持生产 no-go
...
## D095 - 生产替换以 run 前 engine 冻结、隔离 attempt 和服务端 kill switch 分阶段推进
...
- 影响：D095 先冻结契约并授权实现默认关闭的 policy/production shadow scaffold，不授权 canary。canary 必须持有内容寻址 `PromotionAuthorizationV1`，绑定 D094 suite、完整独立人工盲评、durable registry、direct implementation identity 与允许的最大比例；缺任一项 fail-closed。父 manifest/API 后续升级为带 `engine/representation` discriminator 的兼容 union，公开 Artifact 白名单不扩大，完整 LayerPlan/ProgramSpec/Prompt 保持私有。任何 Graph 节点/边/路由或注册变化必须同步 Builder ASCII、Graph Mermaid、路由表并通过 docs-check 与 LangGraph validate；默认 engine 的最终切换必须再立 ADR，`shader_graph_v1`、历史 reader、测试和 kill switch 永久保留。
- 实现更新（2026-07-27，runtime authorization guard）：policy 内嵌授权只作为待验证声明；`canary/direct_default` 的 Backend 启动配置必须额外读取显式受信 registry，把 D094 suite、递归 verifier、人工结果、目标 stage、不可变 bundle URI/hash 和当前 direct implementation identity 与唯一 durable entry 逐字段匹配，并冻结 authorization/registry hash 回执。缺 entry、`partial`、symlink、重复 key/id 或任一漂移均启动 fail-closed；启动期不联网拉取私有 bundle，上传后内容复验是登记与签发前置责任。当前 registry 没有 durable promotion entry，所以此 guard 不授权或启用 canary。
- 回滚修订（2026-07-27）：kill switch 必须高于 promotion runtime verification，否则 registry 或 durable 存储故障会让紧急回滚进程无法启动。`SHADERGEN_DIRECT_GLSL_KILL_SWITCH=1` 时仍严格解析整份 policy YAML，但有效 stage 先降为 `disabled`，允许跳过 registry 和授权回执；开关回到 `0` 后，configured `canary/direct_default` 重新要求完整校验。该修订只恢复旧引擎启动能力，不授权 canary。
- Artifact 安全修订（2026-07-27）：rollout direct/private-old child 使用显式 restrictive `LocalArtifactStore`，只对私有根把目录/文件收紧为 0700/0600，不迁移历史 public store 默认权限；父发布前强制 engine/representation 与 `engine_run.selected_representation` 三方一致。公开 final bundle 改用 pinned dir fd、`O_NOFOLLOW` 与 inode/时间快照复验，消除原先普通 `read_bytes` 可能先跟随 symlink 才拒绝的窗口；本地根仍必须由单一服务进程独占。
- 实现更新（2026-07-27，authority runtime 完成但不可启用）：Backend 已接入父 run 协调器、真实 owned direct child、独立 private ShaderGraph fallback child、确定性 UUID5 attempt、write-once claim/失败安全摘要、原子父 Artifact v2 发布、历史 v1 reader 回退和 API/Frontend discriminator。每个 direct/fallback attempt 使用独立 Renderer/cache/预算和 private store；direct 失败只能创建新 old child，未选 child 不进入公开 index。该实现不修改产品 Graph 拓扑，也不等价于签发 production authority：当前 registry 无 durable promotion entry，故真实 canary/direct-default 配置仍启动 fail-closed，默认 `shader_graph_v1` 不变。

## D096 - 人工盲评通过并冻结本地 promotion bundle，生产仍等待 durable
...
## D098 - pre-production 单人开发采用风险分级验证，不默认扩建质量与发布治理

- 日期：2026-07-27
- 决策：当前项目明确处于单人、未上线的快速迭代阶段。普通改动只执行与改动直接相关的聚焦测试；跨组件行为增加一条代表性 happy-path 集成或 E2E；缺陷修复覆盖对应回归路径。全量 `make check`、全量集成/浏览器验收只在阶段里程碑、准备合并/发布、公共基础设施变更或用户明确要求时运行。真实模型 benchmark、A/B、shadow、人工盲评、durable evidence、promotion authorization、canary 和其他生产治理只在用户明确发起方案比较或上线准备时推进。
- 原因：此前把“代码正确、链路完整”扩大为统计质量证明和生产晋升，导致验证与治理工作量超过功能实现，不符合当前责任主体、环境数量和迭代目标。已有测试资产足以支持按改动范围做回归；历史实验和安全实现可以保留，但没有必要成为每个功能的默认完成条件。
- 影响：`AGENTS.md` 成为日常验证强度的直接规则；`PROGRESS.md` 将 benchmark、人工门禁、durable registry、canary、多环境可靠性和完整兼容矩阵移入“上线前待办”；`docs/FEATURES.md` 区分日常增量验证与功能转为 `passing` 的里程碑验证。D076、D088、D090、D093–D096 的历史结论及产物不删除，但其后续实验/晋升要求在当前阶段休眠。若未来用户明确进入上线准备阶段，应新增 ADR 恢复相应门禁，而不是由 Agent 根据“可发布”“生产”等模糊措辞自行启动。

## D099 - 文档按实时事实、历史证据和参考材料分层

- 日期：2026-07-28
- 决策：当前开发只从 `AGENTS.md`、根/模块 README、根/模块 `ARCHITECTURE.md`、`docs/FEATURES.md`、`PROGRESS.md` 和 ADR 状态索引读取约束与事实。`docs/superpowers/plans/`、`docs/superpowers/specs/`、`docs/analysis/` 和 `human_doc/` 的文档必须在首屏明确标为历史、参考或当前可选工具；其正文中的“下一步”“最终”“权威”“门禁”和测试命令不能自动生成当前任务。ADR 状态索引必须覆盖全部 ADR，禁止“未列出即默认 accepted”。`docs-check` 对这些边界做静态检查。
- 原因：仓库长期保留了大量真实实验、阶段计划和目标架构。它们本身有追溯价值，但旧正文使用现在时和命令式语气，导致 Agent 在后续开发中把已经完成、淘汰或休眠的 benchmark、A/B、全量测试和治理流程重新执行，直接违背 D098 的快速迭代目标。
- 影响：历史内容和失败证据不删除，只降低执行权威；发生冲突时实时事实文档优先。普通文档修订只运行 `make docs-check` 和必要的格式检查，不因清理历史叙述而启动全量测试、真实模型实验或 benchmark。新增历史/参考文档如果缺少状态标记，或新增 ADR 未加入索引，`docs-check` 必须失败。
