...
> 归档状态：历史 evidence 治理说明。Registry 当前休眠；未来进入上线准备时应按新的当前决策重新定义使用方式。
...
- 未来显式发布 gate 只有在对应 evidence 达到 `durable` 且届时定义的功能验证命令通过后才能作为发布证据；当前 `partial` 只表示历史材料不完整，不是 F09 日常开发缺口。
...
获得权限。用于显式 `canary` 或携带授权的 `direct_default` 的 registry entry 必须使用
