...
- 实现更新（2026-07-27，runtime authorization guard）：policy 内嵌授权只作为待验证声明；`canary/direct_default` 的 Backend 启动配置必须额外读取显式受信 registry，把 D090 suite、递归 verifier、人工结果、目标 stage、不可变 bundle URI/hash 和当前 direct implementation identity 与唯一 durable entry 逐字段匹配，并冻结 authorization/registry hash 回执。缺 entry、`partial`、symlink、重复 key/id 或任一漂移均启动 fail-closed；启动期不联网拉取私有 bundle，上传后内容复验是登记与签发前置责任。当前 registry 没有 durable promotion entry，所以此 guard 不授权或启用 canary。
