...
本轮冻结的 v2 自动门禁为 **`supported`**；D092 后续人工门禁也为
**`supported`**，但生产晋升决策仍是 **`no_go_pending_durable`**。

这表示 D088 的 direct GLSL v2 契约稳定性改造已经把实验从“自动门禁失败”
...
3. 生产接入按 D091 在每个 run 启动前冻结 engine；先以完全隔离的 production
