...
def _legacy_diagnostic_dependencies() -> tuple[Any, Any, Any, Any, Any]:
    """延迟加载已退役的旧 MinScene 诊断依赖.

    当前分支只保留纯函数与历史实现用于审计；D076 已停止继续运行旧
    Feature 实验，因此不得为导入本模块而恢复整套 V1 benchmark。
    """
    try:
        calibration = importlib.import_module(
            "scripts.run_scene_mvp_scorer_calibration"
        )
        tile_guard = importlib.import_module("scripts.run_scene_mvp_tile_guard_ab")
        benchmark = importlib.import_module("shaderforge.benchmark")
        evaluation = importlib.import_module("shaderforge.evaluation")
    except (ImportError, ModuleNotFoundError) as exc:
        raise RuntimeError(
            "D076 已退役旧 MinScene benchmark 运行依赖；"
            "当前只保留 acceptance 纯函数、历史源码与既有报告审计。"
        ) from exc
    return (
        benchmark.load_benchmark_suite,
        calibration._regions,
        evaluation.evaluate_render,
        tile_guard.max_tile_regression,
        tile_guard.tile_mae_grid,
    )
...
async def run_acceptance_live_ab(
    ...
    (
        ...
        max_tile_regression,
        tile_mae_grid,
    ) = _legacy_diagnostic_dependencies()
    if output_dir.exists():
        raise FileExistsError(f"输出目录已存在，拒绝覆盖：{output_dir}")
            ...
            for mode in ARM_ORDER:
                ...
                arms_payload[mode] = {
                    ...
                    "tile_summary_vs_fallback": _tile_summary(
                        ...
                        fallback_rgb,
                        arm.final_rgb,
                        max_tile_regression=max_tile_regression,
                        tile_mae_grid=tile_mae_grid,
                    ),
                }
