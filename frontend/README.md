...
- `MinRunLivePanel.tsx` 消费该视图模型；终态后面板冻结为历史记录，状态 live region 不包含每秒计时，完成节点提供可读文本。`App.tsx` 的进度轮询保持 single-flight，并对失败与连续 pending 使用 capped backoff；每次 GET 最多等待 60 秒，事件按 seq 去重排序。生成 POST 的默认浏览器等待按 fast/balanced/high/manual 分别为 5/6/8/12 小时，且 POST 结算（含用户停止等待/超时）后继续有界观察 2 小时，直到服务端终态、带匹配 run_id 与稳定 code/stage 的权威应用失败、run 创建前的 `client_validation/request_validation`、新 run、页面卸载或观察上限；代理错误不会伪装成 run 终态。停止等待明确不是服务端取消。
...
- `VITE_API_BASE_URL` 配置产品后端，`VITE_NODE_LAB_API_BASE_URL` 配置独立 Node Lab；等待窗口统一编辑 `src/shaderforge/config/runtime_timeouts.yaml` 的 `frontend` 段。Vite 启动/构建时严格读取并注入该段，修改后必须重启开发服务器或重新构建。浏览器环境变量都会进入产物，不能包含秘密。
...
`src/**/*.test.{ts,tsx}` 由 Vitest 运行；Node Lab 输入填充纯函数位于
`tests/**/*.test.ts`，使用 Node 内置 test runner 直接运行：
