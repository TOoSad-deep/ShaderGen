...
- 当前公开变量包括产品 `VITE_API_BASE_URL`、独立服务 `VITE_NODE_LAB_API_BASE_URL` 和 `VITE_GENERATION_REQUEST_TIMEOUT_MS`；前两者默认分别指向 `8088` 与 `8090`，超时变量留空时按质量档位使用内置等待上限。

## API 规则

- 所有后端调用集中在 `src/api/`，并经 `src/api/client.ts` 的 `apiFetch()` / `resolveApiUrl()` 访问；组件和页面不得绕过该边界。
- 产品 API base URL 通过 `VITE_API_BASE_URL` 配置，默认指向 `http://127.0.0.1:8088`；Node Lab API 通过 `VITE_NODE_LAB_API_BASE_URL` 配置，默认指向独立服务 `http://127.0.0.1:8090`。两者都只用于本地开发。
...
- `src/api/nodeLab.ts` 只封装独立服务的 `/api/lab/v1/*` 调试边界；`/lab` 工作台读取服务端 descriptor 和调用示例，不在前端复制节点输入规则、Fixture 或 benchmark 判定。步骤列表直接消费服务端返回的 DAG summary，只在用户选中步骤时读取完整 detail，禁止为每个步骤产生 N+1 请求。
...
- 涉及接口字段时，同时检查 `frontend/src/api/` 类型与对应的 `backend/app/schemas/` 或 `src/nodelab_service/schemas.py` 是否一致。
