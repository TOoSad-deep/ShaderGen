...
- `scene_mvp` 在 `ShaderResponse` 上读取可选 `min_pipeline`（`mae`、`render_count`、`llm_call_count`、`scene`、`trace[{phase,status,duration_ms?,message?}]`，以及质量/性能字段 `target_mae`、`target_reached`、`renderer_path`、`prepare_duration_ms`、`uniform_render_count`、`uniform_render_p95_ms`）；所有字段按可缺省处理，缺省时展示占位而不是报错。
...
- `scene_mvp` 结果由 `SceneMvpSummary` 纯展示组件渲染：MAE 与目标 MAE、基于 `target_reached` 的“质量达标/流程完成未达标”提示（区分流程完成与质量达标，字段缺省时不下结论）、渲染/LLM 计数、run_id、停止原因、prepared 渲染路径、prepare 耗时、uniform 热渲染次数与 P95，以及可折叠的场景 JSON 和阶段追踪；最终 Render 与 GLSL 预览继续复用既有面板。
