...
def _no_validated_shader_http_error(
    exc: NoValidatedShaderError,
    *,
    run_id: UUID,
) -> HTTPException:
    """按停止原因和安全诊断把闭环失败映射到 HTTP 语义."""
...
async def _record_generation_failure_without_masking(
    pool: Any,
    *,
    run_id: UUID,
    project_id: UUID,
...
async def _record_generation_success_without_masking(
    pool: Any,
    *,
    run_id: UUID,
    project_id: UUID,
...
async def _start_generation_run_or_raise(
    pool: Any,
    **start_kwargs: Any,
) -> None:
...
async def generate_shader(
    ...
    try:
        ...
        raise _no_validated_shader_http_error(exc, run_id=run_id) from exc
