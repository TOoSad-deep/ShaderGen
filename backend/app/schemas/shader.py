...
ShaderEngineId = Literal["direct_glsl_layerplan_v1"]
ShaderRepresentation = Literal["shader_program_spec_v1"]
...
class ShaderEngineRunSummary(BaseModel):
    """Direct parent run and isolated attempt summary."""
...
class ShaderMinPipelineSummary(BaseModel):
    ...
    renderer_path: Literal["direct_program_spec_v1"]
    target_mae: float
    target_loss: float
    target_reached: bool
...
class ShaderResponse(BaseModel):
    ...
    engine: ShaderEngineId
    representation: ShaderRepresentation
    engine_run: ShaderEngineRunSummary
