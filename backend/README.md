...
- `scene_mvp` 复用同一 project/run 总账，不新增数据库表；run 身份、实验 ID、报告版本、配置指纹、MAE/loss 目标与 `fast|balanced|high` 的 render/LLM/Refine 有界预算从 `src/agent/app/config/png_to_shader_min.yaml` 启动加载。当前 `0.04/0.02` 与 `48/2/1`、`96/4/2`、`640/9/9` 显式属于独立实验；声明冻结 benchmark 时如偏离 D058/D059 口径则启动失败。每档 Graph recursion limit 按 `9+2F+6R` 推导；意外递归失败仍返回 500 并写入不含图片、Scene、GLSL 或输入内容的进度安全快照。模型失败回退确定性感知 scene。run result 记录实际身份/配置、状态、停止原因、整图 MAE、局部复合 loss、预算/实际用量、scene、阶段 trace、Patch 安全证据、公开 Artifact URL及 `prepared_uniforms_v1` 摘要，阶段 trace 同步写入 `agent_events`。当 `llm_call_count=0` 时不伪造默认 `model_call` 事件。
