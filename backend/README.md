...
- `scene_mvp` 复用同一 project/run 总账，不新增数据库表；run 身份、实验 ID、报告版本、配置指纹、MAE/loss 目标与 `fast|balanced|high|manual` 的 render/LLM/Refine 有界预算从 `src/agent/app/config/png_to_shader_min.yaml` 启动加载。当前 `0.04/0.02` 与 `48/2/1`、`96/4/2`、`640/9/9`、manual `1000/32/30` 显式属于独立实验；声明冻结 benchmark 时必须回到 D058/D059 三档口径且禁止 manual。每档 Graph recursion limit 按 `9+2F+6R` 推导；意外递归失败仍返回 500 并写入不含图片、Scene、GLSL 或输入内容的进度安全快照。模型失败回退确定性感知 scene。run result 记录实际身份/配置、状态、停止原因、整图 MAE、局部复合 loss、预算/实际用量、scene、阶段 trace、Patch 安全证据、公开 Artifact URL及 `prepared_uniforms_v1` 摘要，阶段 trace 同步写入 `agent_events`。当 `llm_call_count=0` 时不伪造默认 `model_call` 事件。
...
- `POST /api/shader/generate` 接收可选 `project_id`、可选 `run_id`、`generation_mode=procedural_v1|scene_mvp`、`quality_preset=fast|balanced|high|manual` 和最长 2,000 字的 `instruction`；未提供 project/run 时创建 UUID，未提供 mode 时继续默认 `procedural_v1`。`manual` 只允许 `scene_mvp` 独立实验，`procedural_v1 + manual` 返回 422 `invalid_quality_preset`。客户端显式携带 `run_id` 后可在 POST 阻塞期间轮询运行进度；相同 run_id 已有进行中运行时返回 409 `run_conflict`。响应回显实际档位。
...
- `scene_mvp` 响应复用 `ShaderResponse`，并额外返回 `min_pipeline` 摘要：整图 `mae`、`objective_loss`、global/foreground/background/geometry/edge/worst-tile `metric_breakdown`、`template_version`、run classification/experiment/report/config fingerprint、render/LLM/Refine 预算与实际用量、单 Patch 12 draw 上限及脱敏 Patch 证据、`renderer_path="prepared_uniforms_v1"`、目标/达标状态、prepare/uniform 性能、最终 scene 与阶段 trace；同一字段集同步进入 metrics/manifest/运行总账。Patch 证据禁止包含完整 value、图片、GLSL、用户输入、模型原始响应或 reasoning。`score`、`review` 和 V1 候选字段保持空值。
