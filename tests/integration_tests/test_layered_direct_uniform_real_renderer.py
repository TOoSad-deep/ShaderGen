...
from backend.app.services.engine_rollout_runtime import _write_private_process_renders
...
@pytest.mark.anyio
async def test_multi_parameter_uniform_search_uses_real_webgl1_full_chain(
    tmp_path: Path,
) -> None:
    ...
    retained = _write_private_process_renders(private_run, result)
    assert [item["role"] for item in retained] == ["initial"]
    assert retained[0]["became_current_best"] is True
    assert retained[0]["is_final_best"] is False
    assert retained[0]["sequence"] == initial.sequence
    assert private_run.read_bytes(retained[0]["relative_path"]) == initial.png_bytes
