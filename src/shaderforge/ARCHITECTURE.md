...
- `evaluation/`：sRGB RMSE/MAE、边缘、几何、代表像素、ROI 与保护区域 Basic Oracle，Candidate/current_best、runtime structure 重放、typed Candidate 恢复和默认关闭的 sealed admission；
- `genome/`：16 类 sealed Effect Node、typed ports/bindings、显式 SDF→mask AA、Mask algebra、参数语义与四类 canonical hash；
- `seeding/`：从 Intent 确定性生成三个 SeedPlan、展开 TypedEffectGenome 并执行真实 semantic/structural diversity gate；
- `compiler/`：稳定拓扑、typed AST、safe stdlib、GLSL ES 100、SourceMap、parameter table、CompilationBundle 与 compiler-defect 门禁；
...
三个模型角色和自动修订 Graph 已在 `src/agent/app/` 的 M2/M3 实现并调用上述公共能力；M4 已通过 Agent Service、Backend 白名单 API 和前端双端复核接入产品路径。M5 已实现 benchmark harness、AI-off smoke、自动门禁与盲评包，最终发布状态仍取决于固定 10 例真实模型结果和独立人工盲评。

V2.0–V2.2 已完成纯契约、Intent、runtime verifier、typed Genome/Seed/Compiler、typed Candidate 重放与 validation 静态聚合门禁。V2.3 正在实现 Graph/State/Routing/Harness；production admission 继续默认关闭，release-held-out 仍保持封存空集。实施状态和冻结边界见 `docs/V2-STRICT-GATES-AND-IMPLEMENTATION.md`。
...
- 需要精确领域类型、单一职责依赖或避免加载无关实现时，可以从有架构文档的 typed 子包公共根导入，例如 `shaderforge.analysis`、`compiler`、`contracts`、`evaluation`、`generation`、`genome`、`intent`、`rendering`、`seeding`、`store`、`validation` 和 `benchmark`。Agent/Backend 产品代码以子包 `__init__.py`/`__all__` 为公共面；仓库内 benchmark 脚本或聚焦测试若必须直接依赖稳定 typed 定义模块，应把该模块纳入对应架构和回归测试，不能任意依赖未声明的私有 helper。
