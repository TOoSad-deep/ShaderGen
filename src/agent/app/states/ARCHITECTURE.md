...
- `pending_replay_step` 和 `replay_step_refs` 也是 `UntrackedValue`，只服务显式 legacy MinScene Builder 的私有 replay 引用；完整 typed Patch、候选 Scene 和渲染只落到 run 的 `private/replay/`，默认 ShaderGraph 产品目前不消费这两个字段。
