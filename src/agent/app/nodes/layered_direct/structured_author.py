...
def _log_invocation_failure(
    *,
    ...
    """Log stable LLM failure classification without provider content."""
...
def _log_parse_failure(*, stage: str, error: ValueError, model_ref: str) -> None:
    """Log parse classification only; model output is sensitive."""
    logger.warning(
        "structured_author.failure event=%s model_ref=%s error_type=%s "
...
async def invoke_repair(
    ...
    try:
        ...
        _log_invocation_failure(
            ...
            error=exc,
            model_ref=repair_options.model_ref,
            retryable=exc.retryable,
        )
        return {
