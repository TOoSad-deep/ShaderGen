...
- LayerPlan `region`、GLSL `v_uv` 与归一化后的 `gl_FragCoord` 固定为同一个左下原点坐标系：x 向右、y 向上，参考图顶部对应较大的 y。视觉分析 Prompt 负责把普通图像自上而下的行坐标转换为 canonical region；Initial/Refine Prompt 禁止把图像行坐标直接当作 `v_uv.y`，并要求保护细长、弯曲或沿轮廓延伸的高光拓扑。
...
- Prompt 主体在 `agent.app.prompts`：`layerplan_visual_analysis_v1.yaml`、`direct_glsl_initial_v2.yaml`、`direct_glsl_refine_v2.yaml`；结构修复复用 `min_author_repair_v2`。
