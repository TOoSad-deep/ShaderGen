...
- `shader_graph_runtime.py` 沿用现有 12 节点拓扑：感知 MinScene 只作为确定性 fallback 输入，随后立即转换为 ShaderDocument；模型文档与 fallback 都经真实 Compiler/Renderer 仲裁。参数优化按稳定 node/layer block 工作，结构 Patch 每轮只分配一个 raw draw 并严格回滚。
