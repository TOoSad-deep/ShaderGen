...
- `png_to_shader_v1_run_nodes.py`：M3 的确定性单职责节点工厂，包括输入规范化/测量、Candidate Artifact、受限本地修复、真实 render/evaluate、current_best 选择输入、best Artifact 重载、Review 持久化和 finalize；不组装模型 Prompt。Evaluator 不可用时可从真实 WebGL 已验证候选生成明确的未评分 fallback。
