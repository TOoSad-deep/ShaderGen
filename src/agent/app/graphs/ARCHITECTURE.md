...
- 黄色节点构成最小图的 `current_best` 安全边界。Initial 的模型 scene 与确定性感知 fallback 在预算允许时都由 `render_and_evaluate` 真实渲染，先按 `min_scene_composite_v3` 选择较优起点；首次基础/特征优化只在复合 loss 严格改善时提交。Refine 的 raw Scene 始终位于独立 branch，不能临时覆盖 best。全部 acceptance 比较统一使用纯函数 `shaderforge.optimization.accepts_strict_total_loss`（仅 `total_loss` 严格改善；生产不存在 geometry-first 字典序 acceptance，`geometry_mask_loss` 只是加权分量与证据 delta，见 D065）。D065 的纯函数收口是行为等价重构，节点、直接边、条件边、路由结果、终止路径与 `current_best` 安全边界均未变，因此 Builder ASCII、上方 Mermaid 与路由表无需变更。
- `author_initial` 通过注入的 `LLMGateway` 请求完整 `png_to_shader_min_scene_v3`，调用或严格解析失败则使用确定性感知 fallback；`author_refine` 只接受一个 add/remove/replace feature 或完整 replace color field 的白名单 patch，并且永远从 `current_best.scene` 派生候选。语义调用与最多一次结构修复共用 `src/agent/app/config/png_to_shader_min.yaml` 中当前质量档位的 LLM 预算；同一 YAML 还必须声明 `frozen_benchmark|independent_experiment` 身份、独立实验 ID、报告版本和实际配置指纹。当前 `0.04/0.02`、`48/2/1`、`96/4/2`、`640/9/9` 与 manual `1000/32/30` 明确属于独立实验；声明冻结 benchmark 时若不精确匹配 D058/D059 的 `0.08/0.04` 与 `48/2/1`、`96/4/2`、`160/6/3`，或携带 manual，启动即失败。
- Service 不再使用固定 Graph recursion limit。每个 run 按 `R=min(refine_budget,max(llm_budget-1,0))` 和最多四个 feature 推导合法最坏路径 `9 + 2F + 6R`，再增加 4 步框架余量；当前 high 档需要 65 步并注入 69，manual 需要 197 步并注入 201。配置加载会拒绝超过全局安全上限 256 的预算组合。`GraphRecursionError` 仍按内部流水线错误 fail-closed，不能静默导出 best，以免掩盖真实路由死循环。
...
- 私有 Patch replay bundle v1（D066）只是 `author_refine`/`render_and_evaluate`/`finalize` 节点内部的本地证据落盘：完整 typed patch、anchor/candidate/raw/matured scene 与渲染只写入 run 目录 `private/replay/`，State 只携带 `pending_replay_step`/`replay_step_refs` 路径-hash 引用，读回只从 `refine_count` 派生路径并经 verified 解码 fail-closed 校验，公开 manifest 仅新增 hash 级 `private_replay_bundle` 摘要。节点、直接边、条件边、路由结果、终止路径与 `current_best` 安全边界均未变，因此 Builder ASCII、上方 Mermaid 与路由表无需变更。
