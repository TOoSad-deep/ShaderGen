...
`src/nodelab_service/` 是 `nodelab` Harness 的独立 FastAPI transport 和进程组合根。它可以不启动产品 Backend、LangGraph Agent、数据库或 ShaderForge 而单独运行；产品 Backend 不注册 `/api/lab/v1/*`，默认端口分别为 `8088` 和 `8090`。
...
可信启动配置 / Application factory
                 |
                 v
nodelab_service.main -> routes -> service -> nodelab.runner
          |              |
          +-> schemas    +-> LabRun / Step / Artifact / benchmark

nodelab_service -X-> agent / backend / shaderforge
backend          -X-> nodelab_service
```
...
- `routes.py`、`schemas.py`：拥有 `/api/lab/v1/*` HTTP 契约，不接受客户端提供的 import 路径、manifest 路径或文件系统路径。
