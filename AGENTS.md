...
- 前端、UI、多模态、图像、视觉效果和视觉验收等任务优先使用 Kimi Code，模型固定为 Kimi K3。K3 的 effort 只使用其支持的 `low`、`high`、`max`，并根据任务难度、风险和所需视觉推理深度动态调整。
- Kimi Code 可以直接通过终端使用，也可以通过 Orca 调度；需要调用时先读取 [Kimi Code 运行手册](docs/runbooks/KIMI_CODE.md)，根据当前任务环境、上下文传递和协作需求选择调用方式。
