...
- 已按 D086 冻结并实现 LayerPlan shadow suite：新 manifest 绑定四张版本中立参考图、统一 instruction、两轮 AB/BA 顺序和共享预算；预声明 gate 绑定 manifest SHA、两种臂序的完整配置指纹、metric、配对改善阈值、顺序效应、inconclusive、人工偏好与 durable 晋升要求。suite 会顺序运行全部样本，递归复验每个单 run 与冻结输入，再按配对 `B-A`/样本中位数/顺序效应/inconclusive 计负聚合并原子写私有报告；自动通过仍保持 pending human + durable no-go。fake LLM + 真实 Chromium 的 2×2 全链已通过。首次 live suite `shadow-suite-4cd3b45d7644` 完整复验通过，但根 `.env` 缺失使 8/8 run 均以 `LLMConfigurationError` fail-closed、0 个候选、inconclusive ratio=1.0，报告 SHA-256 为 `4cd3b45d7644d15a33d0cef6b57e9ab777c53a17f6aa153884e1cdbb25a229f6`；该结果只证明配置失败处理，未消耗有效模型调用，不能用于质量比较。
...
- 在根 `.env` 配置 `SHADER_GEN_MODEL_NAME=kimi:k3-256k` 与 `KIMI_API_KEY` 后，使用新 output root 重跑 D086 四样本 × 两轮真实模型 A/B 并复验私有 suite 报告；不得覆盖首次配置失败证据。在获得 durable、跨环境复验与人工偏好证据前，不修改生产路径。
...
- LayerPlan shadow harness、冻结 manifest/gate、suite 调度/聚合和递归复验已实现，但根 `.env` 当前缺失，首次 live suite 只能产生 8/8 `LLMConfigurationError` 的完整失败证据；尚无有效真实模型 A/B、durable 跨环境证据或人工偏好结论。晋升前不得影响生产候选选择、scorer、预算或 `current_best`。
