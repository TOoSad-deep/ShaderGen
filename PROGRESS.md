...
- 把本轮三个 Qwen 小样例整理为版本中立的固定 canary manifest，保留“模型初稿均输给 fallback”的负面质量事实；再补 1–2 个 Boolean/8-layer 边界样例，不扩大为旧 V1 benchmark。
...
- tile guard offline replay 没有保护收益且存在高误拒；生产 Qwen Provider 已直连，但三个 Initial 模型候选都输给确定性 fallback，说明链路正确不等于 Author 质量达标。固定多样例、Boolean/8-layer 边界和独立人工偏好仍未完成。
...
- 2026-07-24 新增脱敏 repair 诊断后的 `make check` 通过：295 个单元测试、docs-check、LangGraph validate（1 个 Graph）和前端生产构建均成功；`mypy --strict src backend` 通过 100 个源文件。
