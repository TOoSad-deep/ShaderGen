...
- 固定 7 例使用相同候选与 draw 预算，对当前严格 total-loss 接受和 `4×4/8×8` 全 tile 最大回退 guard 做单因素 A/B；记录内部/外部 objective、关键 ROI、误拒率和三列看片，只在 guard 能保护 `ellipse_gradient` 与 `arc_highlight_orb` 且不破坏 `solid_circle`、`color_lobes` 时再设计生产接入。
...
- `min_scene_composite_v3` 已去除亮度分位数伪语义；固定 7 例证明 geometry 对整体轮廓/错误 shadow 有效，但 aggregate loss 会补偿局部结构回退，且不保证 rim/arc/双高光出现。多尺度 tile guard、真实模型固定 7 例和独立人工偏好仍是发布阻塞项。
...
- 2026-07-23 当前工作树通过 `make check`：503 个 Python 单元测试、`docs-check`、LangGraph validate（2 个 Graph）与 Frontend production build 均成功；high=`640/9/9`、manual=`1000/32/30` 的递归预算及 Progress API 集成为 6 passed，`make test-scene-mvp-ui` 已验证 Manual 仅在 scene_mvp 展示且切回 V1 自动回落 High。全仓 Ruff、`mypy --strict src backend`（148 个源文件）、校准脚本 strict mypy 与 `git diff --check` 通过。固定 7 例 scorer 校准真实 Chromium v2 完成 462 draw，报告 SHA-256 为 `722fdb75bfe7402b45299d4d6199bb429392f7cbdd913457d89c248d224dcbdd`；全程未调用模型。
