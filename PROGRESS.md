...
- 私有 replay bundle v1 与合成 fixture 12/32 机制实验已完成（D066/D067）；下一步运行显式真实模型 independent experiment，使用未来 bundle 的同一 typed Patch 重放 12/32，再决定生产 maturity 预算。
...
- 固定生产方向顺序的两个合成 Patch fixture 表明 32 draw 可降低 loss 并 clean rescue overfit 候选，但 Arm-32 内部不劣近乎由前缀+strict best 构造保证，且仍缺未来真实 typed Patch、固定 7 例和人工偏好；不能据此直接修改生产 maturity 策略。
