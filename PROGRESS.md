...
- `F09` 已完成 M0–M5、M6.0 可靠性修复、M6.1 离线质量改进及 Node Lab API v1 阶段 A–D。Node Lab 通用内核已从 Agent 内部目录抽为顶层 `nodelab` typed Python 包，并按 `pipeline_id` 注入 Node/Capability/Fixture/Suite；空 Application 不持有 V1 默认语义。默认 V1 适配层仍暴露 20/20 生产节点、八个 capability 和三个 AI-off suite，当前仍为进程内模块而非独立服务。
...
- 2026-07-21 当前工作树通过 `make check`：416 个 Python 单元测试、`docs-check`、LangGraph validate（1 个 Graph）与 Frontend production build 均成功；Integration 为 27 passed、1 skipped，全仓 Ruff、`mypy --strict src backend` 与 `git diff --check` 通过。`uv build --wheel` 确认顶层 `nodelab` 包及其 `py.typed` 进入发布物。未运行真实模型。
...
- `H02` 权威验收命令 `make benchmark-node-lab-ai-off`、`make benchmark-node-lab-model`、`make test-node-lab-ui` 均通过；最终源码下的 capability/node、scenario、Renderer warm、transport 和离线五角色 run id 分别为 `node-lab-cd526c858f71`、`node-lab-562a047ad746`、`node-lab-818d749f8d90`、`transport-6394015010af`、`node-lab-model-ccf0fb7b1656`。这些结果只覆盖 AI-off、离线 fixture 和页面流程，不构成真实模型质量证明。
