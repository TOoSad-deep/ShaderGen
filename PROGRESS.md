...
- `F09` 已完成 M0–M5、M6.0 可靠性修复、M6.1 离线质量改进及 Node Lab API v1 阶段 A–D。Node Lab 通用内核已从 Agent 内部目录抽为顶层 `nodelab` typed Python 包，自有原子存储且不导入 ShaderForge；它按 `pipeline_id` 注入 Node/Capability/Fixture/Suite，并提供 Provider Builder、callable/context/Runnable Adapter、Command-like 返回归一化、完整 JSON Schema 校验和可注入 State reducer。普通 JSON-safe Node 无需改造即可接入，复杂副作用只需在 Pipeline 保留薄 Adapter。默认 V1 适配层仍暴露 20/20 生产节点、八个 capability 和三个 AI-off suite，当前仍为进程内模块而非独立服务。
...
- 2026-07-22 当前工作树通过 `make check`：423 个 Python 单元测试、`docs-check`、LangGraph validate（1 个 Graph）与 Frontend production build 均成功；Integration 为 27 passed、1 skipped，全仓 Ruff、`mypy --strict src backend` 与 `git diff --check` 通过。`uv build --wheel` 确认顶层 `nodelab` 包、新增 `file_store.py`/`provider.py` 及其 `py.typed` 进入发布物。未运行真实模型。
...
- `H02` 权威验收命令 `make benchmark-node-lab-ai-off`、`make benchmark-node-lab-model`、`make test-node-lab-ui` 均通过；最终源码下的 capability/node、scenario、warm resource、transport 和离线五角色 run id 分别为 `node-lab-4bd4012bdedf`、`node-lab-70003768ca68`、`node-lab-af5c780f3b4c`、`transport-3a5349f7d8f6`、`node-lab-model-55adb592e898`。这些结果只覆盖 AI-off、离线 fixture 和页面流程，不构成真实模型质量证明。
